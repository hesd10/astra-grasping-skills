...
def require_healthy(sample, temperature_limit, temperature_margin, load_limit):
    required = ('Status', 'Present_Temperature', 'Present_Load')
    if any(key not in sample for key in required):
        raise ValueError('Required motor health feedback is missing')
    if not all(math.isfinite(sample[key]) for key in required):
        raise ValueError('Motor health feedback must be finite')
    if not all(math.isfinite(value) for value in
               (temperature_limit, temperature_margin, load_limit)):
        raise ValueError('Motor health bounds must be finite')
    if temperature_margin < 0 or load_limit <= 0:
        raise ValueError('Health margin must be nonnegative and effort bound positive')
    if temperature_margin >= temperature_limit:
        raise ValueError('Health margin must leave a positive operating interval')
    if sample['Status']:
