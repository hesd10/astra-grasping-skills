...
try:
    ...
    s=snapshot(); record('ready',s); capture('loaded_resume_ready')
    ...
    while True:
        ...
        try:
            ...
            op=cmd['op']
            if op=='finish':
                capture('final'); record('final',snapshot()); print('FINISHED',flush=True); break
            if op=='observe':
            ...
            if op=='move':
                ...
                startgoals=buses['left'].sync_read('Goal_Position',normalize=False)
                ...
                while True:
                    ...
                    values={n:round(startgoals[n]+alpha*(target[n]-startgoals[n])) for n in target}
                    buses['left'].sync_write('Goal_Position',values,normalize=False); goals.update(values)
                    s=snapshot(); record('motion',s); check(s)
                    if cmd.get('minimum_preload') is not None and s['left']['Present_Load']['6'] < cmd['minimum_preload']:
                        raise RuntimeError('Gripper preload loss during lift')
                    if alpha>=1: break
                    time.sleep(0.08)
